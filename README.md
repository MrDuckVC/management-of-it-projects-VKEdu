...
Для создания нашего MVP мы выбираем **Agile (гибкую методологию с короткими спринтами)**, ориентированную на непрерывную проверку продуктовых гипотез (HADI-циклы).

**Почему это идеально подходит нашему продукту:**

* **Специфика B2C рынка:** Наш продукт нацелен на обычных пользователей, решения принимаются эмоционально и быстро, поэтому критически важен идеальный путь пользователя (User Journey). Короткие спринты позволят нам быстро собирать обратную связь, тестировать гипотезы и бороться за каждый процент конверсии.
* **Итеративность и низкий Time-to-Market:** MVP должен предотвратить страх перед «незавершенным проектом». Кросс-функциональная Agile-команда сфокусируется на доставке бизнес-ценности в кратчайшие сроки, чтобы как можно быстрее выпустить "скелет" продукта и начать цикл измерения-обучения.
* **Гибкость при работе с ИИ:** Так как "мозг" нашего приложения основан на внешних LLM, нам придется постоянно на ходу адаптировать алгоритмы распознавания в зависимости от того, как пользователи будут вводить свои траты текстом. Жесткие методологии вроде Waterfall или V-модели, не терпящие изменений, здесь просто не сработают.
